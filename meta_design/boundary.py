...
class Boundary(fe.SubDomain):
    def __init__(self, mesh, tol=None, **kwargs):
        ...
        if tol is None: tol = fe.DOLFIN_EPS
        self._tol = tol
...
class PeriodicDomain(Boundary):
    def inside(self, x, on_boundary):
        ...
        is_lower_right = self._on_bot(x, on_boundary) and self._on_right(x, on_boundary)
        is_upper_left  = self._on_top(x, on_boundary) and self._on_left(x, on_boundary)
