...
from metatop.optimization.scalar import (DetObjective,
                                         FirstInvariantConstraint,
                                         SecondInvariantConstraint,
                                         ThirdInvariantConstraint,
                                         TraceObjective, UnimodeErrConstraint,
                                         VolumeConstraint)
...
def main(E_max, E_min, nu, start_beta, n_betas, n_epochs, epoch_duration, starting_epoch_duration, extremal_mode, basis_v, nelx, nely, norm_filter_radius, verbose, interim_plot, vector_constraint, tighten_vector_constraint, g_vec_eps, enable_profiling, log_to_db, seed):
    ...
    # ===== Optimizer setup ======
    # f = TraceObjective(ops, verbose=True)
    # f = VolumeObjective(ops)
    f = DetObjective(ops)
    g1 = FirstInvariantConstraint(ops, verbose=True, eps=1.)
    g2 = SecondInvariantConstraint(ops, verbose=True)
    # g3 = ThirdInvariantConstraint(ops, verbose=True, eps=1e-6)
    g4 = UnimodeErrConstraint(ops, verbose=True, eps=1e-6)
    g5 = VolumeConstraint(ops, eps=0.35)
    ...
    # opt.add_inequality_constraint(g3, 0.)
    opt.add_inequality_constraint(g4, 0.)
    opt.add_inequality_constraint(g5, 0.)
